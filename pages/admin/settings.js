...
const Section = ({ title, children }) => (
  <div className="border rounded-2xl p-4 sm:p-6 space-y-4 bg-white">
...
export default function SettingsPage() {
  ...
  const [welcome, setWelcome] = useState(
    "Hi! I’m here to help you with your orders and questions. How can I assist you today?"
  );
  ...
  const saveAll = () => alert("Settings saved ✔️");
        ...
        <div className="grid gap-6 grid-cols-1 xl:grid-cols-2">
          <div className="space-y-6">
            <Section title="Bot Configuration">
              ...
              {[{ label: "Auto‑Response", val: autoResp, setter: setAutoResp },{ label: "Smart Suggestions", val: smartSug, setter: setSmartSug },{ label: "Learning Mode", val: learning, setter: setLearning }].map((t) => (
            ...
            <Section title="Notifications">
              {[{ key: "newMsg", label: "New Messages" },{ key: "orderUpdate", label: "Order Updates" },{ key: "botTransfer", label: "Bot Transfer Alert" },{ key: "emailNotif", label: "Email Notifications" }].map((n) => (
                <div key={n.key} className="flex items-center justify-between text-sm">
                  <span>{n.label}</span>
                  <Toggle enabled={notif[n.key]} setEnabled={(val) => setNotif((prev) => ({ ...prev, [n.key]: val }))} />
                </div>
              ))}
            ...
            <Section title="AI & Automation Settings">
              ...
              <Select label="Response Confidence Threshold" options={["Low (50%)", "Medium (70%)", "High (90%)"]} value={confidence} onChange={setConfidence} />
              <Select label="Max Bot Response Time" options={["Instant", "Within 1 min", "Within 5 min"]} value={responseTime} onChange={setResponseTime} />
            </Section>
          </div>
          ...
          <div className="space-y-6">
            <Section title="Channel Settings">
              <div className="space-y-2">
                {channels.map((c) => (
                  <div key={c.id} className={`flex items-center justify-between px-4 py-2 rounded-lg border ${c.status === "Active" ? "bg-[#f6edff]" : "bg-gray-50"}`}>
                    <span>{c.name}</span>
                    <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${c.status === "Active" ? "bg-green-100 text-green-600" : "bg-yellow-100 text-yellow-700"}`}>{c.status}</span>
                  </div>
                ))}
              </div>
            ...
            <Section title="Team Management">
              ...
              <Select label="Auto Assignment" options={["Round Robin", "Least Busy", "First Available"]} value={autoAssign} onChange={setAutoAssign} />
            </Section>
          </div>
        </div>
